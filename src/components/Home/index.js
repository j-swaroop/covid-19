...
class Home extends Component{
    ...
    renderSearchSuggestions = () => {
      ...
      return(
        <ul className='search-results-container' testid='searchResultsUnorderedList'>
          {searchResults.map(item => <SearchResults 
            stateName={item.state_name} stateCode={item.state_code} 
            key={item.state_code} id={item.state_code} />)}
        </ul>
      )
    }
    ...
    //main
    renderHomeContainer = () => {
        const {isLoading, covidData, searchInput, 
          totalActive, totalConfirmed, totalDecesed, 
          totalRecovered, searchResults} = this.state
